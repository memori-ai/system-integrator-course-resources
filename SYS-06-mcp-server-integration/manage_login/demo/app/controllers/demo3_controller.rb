...
class Demo3Controller < ApplicationController
  ...
  def index
    ...
    # Read the current contents of the shared test/ directory
    @workspace_files = list_workspace_files
  end

  # AJAX endpoint: returns only the HTML of the file tree
  def workspace_files
    @workspace_files = list_workspace_files
  ...
  # Recursively scan directory and return the nested array structure.
